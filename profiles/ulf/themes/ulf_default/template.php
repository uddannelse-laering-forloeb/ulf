    ...
    if (isset($variables['page']['content']['system_main']['nodes'])
      && count(
        ...
        if (isset($node['#node']->field_full_description['und'][0]['value'])) {
            ...
            $meta_description_cut =
              substr($teaser, 0, strpos($teaser, '.')).'...';
...
function ulf_default_menu_link__main_menu($variables)
{
    ...
    return "<li".drupal_attributes($element['#attributes']).">".$link.$sub_menu
      ."</li>";
}
...
function ulf_default_preprocess_node(&$variables)
{
            ...
            if (!empty($variables['field_target_group'])) {
                ...
                if ($variables['view_mode'] == 'print') {
                    ...
                    if (isset($variables['field_target_group']['0'])) {
                        $term =
                          $variables['field_target_group']['0']['taxonomy_term'];
                    }
                }
                ...
                if (isset($term)) {
                    ...
                    $variables['group_type'] =
                      strtolower($term_wrapper->name->value());
                }
            }
    ...
    if (($variables['type'] == 'course'
      ...
      && ($variables['view_mode'] == 'full'
        ...
        $variables['profile_phone'] =
          $author_wrapper->field_profile_phone->value();
        ...
        $variables['profile_address'] =
          isset($account->location['street']) ? $account->location['street']
            : '';
        $variables['profile_postal_code'] =
          isset($account->location['postal_code'])
            ? $account->location['postal_code'] : '';
        $variables['profile_city'] =
          isset($account->location['city']) ? $account->location['city'] : '';
    }

    // Transport application form
    $variables['transport_form'] =
      module_exists('transportpulje_form') ? true : false;
}
...
function ulf_default_preprocess_user_profile(&$variables)
{
    ...
    $variables['location'] = [
      'name' => isset($account->location['name']) ? $account->location['name'] : null,
      'street' => isset($account->location['street']) ? $account->location['street'] : null,
      'postal_code' => isset($account->location['postal_code']) ? $account->location['postal_code'] : null,
      'city' => isset($account->location['city']) ? $account->location['city'] : null,
    ];
}
...
function ulf_default_preprocess_panels_pane(&$variables)
{
    ...
    $variables['theme_hook_suggestions'][] =
      'panels_pane__'.str_replace('-', '__', $variables['pane']->subtype);
    $variables['theme_hook_suggestions'][] =
      'panels_pane__'.$variables['pane']->panel.'__'.str_replace(
        '-',
        '__',
    ...
    if ($variables['display']->layout == 'two_split') {
        $variables['theme_hook_suggestions'][] =
          'panels_pane__'.$variables['display']->layout.'__'.str_replace(
            "-",
            "_",
            $variables['pane']->subtype
          );
...
{
    $is_pager =
      (in_array('pager', $variables['attributes']['class'])) ? true : false;
...
function ulf_default_date_display_range($variables)
{
    ...
    $start_date =
      '<span class="date-display-start"'.drupal_attributes($attributes_start)
      .'>'.$date1.'</span>';
    $end_date =
      '<span class="date-display-end"'.drupal_attributes($attributes_end).'>'
      .$date2.$timezone.'</span>';
